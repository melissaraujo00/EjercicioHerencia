...
class Cocina extends Menu {
    ...
    marcarPedidoListoDespuesDe5Segundos() {
        if (this._estado === "En cocina") {
            setTimeout(() => {
                document.getElementById('estado').innerText = "Listo";
                alert(`Cliente ${this._nombre} ${this._cantidadComida} ordenes de ${this._comida} + ${this._cantidadSoda} Sodas de ${this._soda} su pedido está listo.`);
            }, 5000); 
        } else {
...
function Ordenar() {
    ...
    if (typeof totalPedido !== 'undefined') {
        ...
        cocina.marcarPedidoListoDespuesDe5Segundos();
    }
}
